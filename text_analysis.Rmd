...
sentiment_rec <- sentiment_rec %>% 
  ...
  # Only keep the 500 most common tokens
...
## Add down sampling to sentiments recipe
d_sentiment_rec <- sentiment_rec %>% 
...
multi_lasso_rs %>% 
  collect_predictions() %>% 
  # tuning parameter with best accuracy
  filter(penalty == best_acc$penalty[1])
...
## Test model with downsampling

* Maybe helps to figth imbalanced data

```{r}

multi_lasso_wf <- workflow() %>%
  add_recipe(d_sentiment_rec, blueprint = sparse_bp) %>%
  add_model(multi_spec)
